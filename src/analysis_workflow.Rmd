...
continent_anv <- anova(lm_confound_not, lm_confound, test = "LRT")
# 
# df_continent_anv<- summary(continent_anv) %>% broom::tidy()
# df_continent_anv %>% select(Var2, Freq) %>% spread(Var2, Freq) %>%  str
# knitr::kable(df_continent_anv)
# df_continent_anv
```
...
```{r}
ggplot(data = survey_results_continent, aes(x = consumption_goods, y = group_bin, group = group, colour = group)) + 
  geom_jitter(alpha = 0.2, aes(fill = group)) + 
