...
Above we see that that the job satisfaction confounder variable does contribute towards the correlation between savings and salary ratio.

```{r}
ggplot(survey_results, aes(y = ratio, x = savings, group = no_increase_acceptance, colour = no_increase_acceptance)) + 
  geom_point(aes(fill = no_increase_acceptance), alpha = 0.2) + 
  geom_smooth(method = "lm")
```


There seems to be a positive correlation between the percentage a person contributes towards savings and the person's salary ratio **only** when a person person prefers a salary increase above high job satisfaction.

We saw earlier that participants with different preferences in terms of job satisfaction and salary increases have more or less the same salary ratio distribution. However, our above findings is an indication that the preference for a salary increase does have play a role on the relationship between savings contribution and salary ratio, opposed to those who prefer a high job satisfaction.

We aren't directly interested in a person's preference between job satisfaction and salary increase, but we do need to take into account how this variable is influencing our study. There are various ways of dealing with confounding variables, but given our our dataset size, our options are limited. For now, including this interaction in our model should be sufficient to maintain awareness of its effect.

Another interaction that has a fairly low p-value is the interaction between living expenses and job satisfaction preference.

```{r}
ggplot(survey_results, aes(y = ratio, x = living_expenses, group = no_increase_acceptance, colour = no_increase_acceptance)) + 
  geom_point(aes(fill = no_increase_acceptance), alpha = 0.2) + 
  geom_smooth(method = "lm")
```

It would appear that people who spend a small percentage on living expenses have a larger salary ratio **only** when they prefer a salary increase. Living expenses is defined as basic needs expenses. This observation is indeed through provoking. The confidence intervals are fairly wide, but there might be some truth in the finding. It could contribute towards our hypothesis - people who spend a large percentage on non-essentials (i.e. non-living expenses) may be the people who are driven by money. In this case, it seems as if our confounding variable interaction could support our hypothesis (we have to keep in mind that savings is a non-essential and not necessarily an indication of high social standards). However, the lack of statistical significance and small number of observations mean that we cannot draw any conclusions. However, it is important to differentiate between the people who prefer job satisfaction and those who prefer an increase.

We have observations from all across the world. 
