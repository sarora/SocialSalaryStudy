...
The data is anonymized before being made visible and then uploaded as a processed [dataset](https://github.ubc.ca/ubc-mds-2017/SocialSalaryStudy/blob/vmulholl/data/processed/surveydata_clean.rds).

Several pre-processing steps are performed to clean and wrangle the data into a usable format. Pre-processing included removing salary ratio outliers that may have been caused by incorrect data entries. It was decided to remove the values beyond ~95% confidence level. The box-plot method performs a more sophisticated outlier selection than the alternative, the quantile approach, that is more rigid in the 95% threshold. Since we have less observations than ideal, it seemed more appropriate.
